...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($espacio_id) || empty($fecha) || empty($hora_inicio) || empty($hora_fin)) {
        ...
        // Crear reserva
        $stmt = $pdo->prepare("INSERT INTO reservas (espacio_id, usuario_id, fecha, hora_inicio, hora_fin, nombre_contacto, telefono, numero_identificacion, estado) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'CONFIRMADA')");
        if ($stmt->execute([$espacio_id, $usuario['id'], $fecha, $hora_inicio, $hora_fin, $nombre_contacto, $telefono, $numero_identificacion])) {
